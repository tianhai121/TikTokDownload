        ...
        for i in range(len(self.author_list)):
            ...
            try:
                ...
                jx_url = f'https://www.iesdouyin.com/aweme/v1/web/aweme/detail/?aweme_id=' \
                         f'{self.aweme_id[i]}&aid=1128&version_name=23.5.0&device_platform=android&os_version=2333'
                js = Util.json.loads(
                    Util.requests.get(
                        url=jx_url, headers=self.headers
                    ).text
                    )
            ...
            try:
                if self.music == "yes":  # 保留音频
                    ...
                    content_size = int(
                        music.headers['content-length']
                    )  # 下载文件总大小
                    if music.status_code == 200:  # 判断是否响应成功
                        print(
                            '[  音频  ]:' + creat_time + self.author_list[i] + '[文件 大小]:{size:.2f} MB'.format(
            ...
            try:  # 生成1080p视频链接
                self.new_video_list.append(
                    'https://aweme.snssdk.com/aweme/v1/play/?video_id=%s&ratio=1080p&line=0' % self.uri_list[i]
                    )
                t_video = Util.requests.get(
                    url=self.new_video_list[0],
                ...
                chunk_size = 1024  # 每次下载的数据大小
                content_size = int(
                    t_video.headers['content-length']
                )  # 下载文件总大小
                try:
                    if t_video.status_code == 200:  # 判断是否响应成功
                        print(
                            '[  视频  ]:' + creat_time + self.author_list[i] + '[文件 大小]:{size:.2f} MB'.format(
                                size=content_size / chunk_size / 1024
                            )
                            )  # 开始下载，显示下载文件大小

                        v_url = self.path + self.sprit + creat_time + Util.re.sub(
                            r'[\\/:*?"<>|\r\n] + ', "_", self.author_list[i]
                        ) + '.mp4'
